...
describe('LogContext', () => {
	...
	describe('deleteLog', () => {
		it('should remove log by id', async () => {
			...
			mockAsyncStorage.getItem.mockResolvedValue(JSON.stringify([mockLog, log2]));
			mockAsyncStorage.setItem.mockResolvedValue();
	...
	describe('clearAllLogs', () => {
		it('should clear all logs successfully', async () => {
			...
			expect(mockAlert.alert).toHaveBeenCalledWith('完了', 'すべてのデータを削除しました。');
		});
		...
		it('should handle clear error', async () => {
			...
			expect(mockAlert.alert).toHaveBeenCalledWith('エラー', 'データの削除に失敗しました。');
		});
	});
