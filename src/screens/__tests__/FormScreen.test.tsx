...
describe('FormScreen', () => {
	...
	describe('rendering', () => {
		it('should render form with empty fields for new log', () => {
			...
			expect(screen.getByPlaceholderText('例：朝の会議で発言できなかった')).toBeTruthy();
			expect(screen.getByPlaceholderText('例：「どうせ自分は評価されていないんだ」')).toBeTruthy();
		});
	...
	describe('interactions', () => {
		...
		it('should call onSave with new log data when save button is pressed', () => {
			...
			const situationInput = screen.getByPlaceholderText('例：朝の会議で発言できなかった');
			const thoughtInput = screen.getByPlaceholderText('例：「どうせ自分は評価されていないんだ」');
		...
		it('should call onDelete when delete is confirmed', () => {
			...
			const deleteConfirmButton = alertCall[2].find(button => button.text === '削除する');
